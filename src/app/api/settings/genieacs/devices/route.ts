...
export async function GET(request: NextRequest) {
  try {
    ...
    // Use comma-separated projection (GenieACS NBI doesn't support repeated &projection=)
    ...
    const fetchDevices = async (pageLimit: number, pageSkip: number) => {
      ...
      // Don't use projParam with query - GenieACS NBI has issues with query + multi-projection
    ...
    const fetchStats = async () => {
      ...
      const totalCount = parseInt(countRes.headers.get('total') || '0');
      const idsRes = await fetch(`${host}/devices?limit=${Math.min(totalCount, 1000)}&projection=_id,_lastInform`,
      ...
      return { total: totalCount, online, offline };
    };
